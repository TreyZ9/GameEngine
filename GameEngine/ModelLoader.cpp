...
std::vector<Model> ModelLoader::loadModels(const std::string& filename)
{
	...
	while (std::getline(infile, line))
	{
		if (!line.empty())
		{
			std::vector<std::string> elements = ModelLoader::splitString(line, ":");
			if (elements[0] == "model")
			{
				if (elements[1] != "end")
...
std::vector<std::string> ModelLoader::splitString(std::string input, std::string delimiter)
{
